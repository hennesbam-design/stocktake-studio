...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/areas", async (req, res) => {
    try {
      ...
      const areas = await storage.getAreas(departmentId as string);
      res.json(areas);
    } catch (error) {
  ...
  app.get("/api/product-groups", async (req, res) => {
    try {
      ...
      const groups = await storage.getProductGroups(areaId as string);
      res.json(groups);
    } catch (error) {
  ...
  app.get("/api/items", async (req, res) => {
    try {
      ...
      const items = await storage.getItems(groupId as string);
      res.json(items);
    } catch (error) {
  ...
  app.get("/api/entries", async (req, res) => {
    try {
      ...
      const entries = await storage.getEntries(sessionId as string);
      res.json(entries);
    } catch (error) {
