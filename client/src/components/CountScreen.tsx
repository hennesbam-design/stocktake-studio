...
interface CountScreenProps {
  ...
  onBack: () => void;
  onScanBarcode: () => void;
  onAddEntry: (entry: { itemId: string; fulls: number; singles: number; notes: string }) => void;
}
...
export function CountScreen({
  ...
  const adjustCount = (type: 'fulls' | 'singles', delta: number) => {
    if (type === 'fulls') {
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        size="icon"
                        variant="outline"
                        onClick={() => adjustCount('fulls', -1)}
                        data-testid="button-fulls-minus"
                      >
                      ...
                      <Input
                        type="number"
                        value={fulls}
                        onChange={(e) => setFulls(Math.max(0, parseInt(e.target.value) || 0))}
                        className="text-center h-12 text-xl font-medium"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        size="icon"
                        variant="outline"
                        onClick={() => adjustCount('singles', -1)}
                        data-testid="button-singles-minus"
                      >
                        <MinusIcon className="h-4 w-4" />
                      </Button>
                      <Input
                        type="number"
                        value={singles}
                        onChange={(e) => setSingles(Math.max(0, parseInt(e.target.value) || 0))}
                        className="text-center h-12 text-xl font-medium"
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => adjustCount('singles', 1)}
                        data-testid="button-singles-plus"
                      >
                        <PlusIcon className="h-4 w-4" />
                      </Button>
                    </div>
