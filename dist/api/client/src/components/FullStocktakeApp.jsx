...
import { SearchIcon, ScanLineIcon, PlusIcon, ArrowLeftIcon, PackageIcon, HomeIcon } from "lucide-react";
...
export function FullStocktakeApp() {
    const { currentScreen, setCurrentScreen, operatorName, setOperatorName, selectedDepartment, setSelectedDepartment, adminPin, setAdminPin } = useStocktakeStore();
    ...
    const handleWelcomeContinue = (operator1, operator2, date) => {
    ...
    const handleExportCSV = () => {
        try {
            ...
            const rows = entries.map((entry) => {
                ...
                const area = areas.find((area) => item && area.id === item.areaId);
                const department = departments.find((dept) => area && dept.id === area.departmentId);
                return [
                    entry.id || '',
                    entry.sessionId || '',
    ...
    const handleClearData = async () => {
        ...
        const userConfirmed = window.confirm("WARNING: This will delete ALL stocktake data including departments, areas, items, sessions, and entries. This action cannot be undone. Are you sure you want to continue?");
        if (!userConfirmed) {
            return;
        }
        try {
    ...
    const renderHeader = () => (<div className="sticky top-0 z-10 bg-background border-b px-4 py-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          {(currentView !== 'departments' || selectedDepartment || selectedArea) && (<Button variant="ghost" size="icon" onClick={() => {
          ...
          <div>
            ...
            {operatorName && (<p className="text-sm text-muted-foreground">
                Operator: {operatorName}
              </p>)}
          </div>
        </div>

        <MenuDrawer onExportCSV={handleExportCSV} onExportJSON={handleExportJSON} onImportCSV={handleCsvImport} onClearData={handleClearData} onDownloadTemplate={handleDownloadTemplate} adminPin={adminPin} onSetAdminPin={setAdminPin}/>
      </div>
    </div>);
    // Render departments view
    const renderDepartmentView = () => (<div className="p-4 space-y-4">
      ...
      {departmentsLoading ? (<div className="text-center py-8">Loading departments...</div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    // Render areas view
    const renderAreaView = () => (<div className="p-4 space-y-4">
      <div className="flex justify-between items-center">
      ...
      {areasLoading ? (<div className="text-center py-8">Loading areas...</div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    // Render items view
    const renderItemView = () => (<div className="p-4 space-y-4">
      <div className="flex gap-2 mb-4">
        <div className="relative flex-1">
          <SearchIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
          <Input type="text" placeholder="Search items or scan barcode..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" data-testid="input-search-items"/>
        </div>
      ...
      {itemsLoading ? (<div className="text-center py-8">Loading items...</div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredItems.map((item) => (<Card key={item.id} className="hover-elevate cursor-pointer" onClick={() => {
              ...
              <CardContent className="space-y-2">
                {item.barcode && (<p className="text-sm font-mono text-muted-foreground">
                    {item.barcode}
                  </p>)}
                {item.packVolume && (<Badge variant="outline">{item.packVolume}</Badge>)}
              </CardContent>
            </Card>))}
        </div>)}
    </div>);
    // Render count view
    const renderCountView = () => (<div className="p-4 space-y-6">
      {selectedItem && (<Card>
          <CardHeader>
            <CardTitle>{selectedItem.name}</CardTitle>
            {selectedItem.barcode && (<p className="text-sm font-mono text-muted-foreground">
                {selectedItem.barcode}
              </p>)}
            {selectedItem.packVolume && (<Badge variant="outline">{selectedItem.packVolume}</Badge>)}
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-medium">Fulls</label>
                <Input type="number" min="0" value={fulls} onChange={(e) => setFulls(e.target.value)} placeholder="0" className="text-center text-lg" data-testid="input-fulls"/>
              </div>
              
              <div className="space-y-2">
                <label className="text-sm font-medium">Singles</label>
                <Input type="number" min="0" value={singles} onChange={(e) => setSingles(e.target.value)} placeholder="0" className="text-center text-lg" data-testid="input-singles"/>
              </div>
            </div>
            
            <Button onClick={handleAddCount} className="w-full" size="lg" data-testid="button-record-count">
              Record Count
            </Button>
          </CardContent>
        </Card>)}
    ...
    const renderScannerModal = () => (scannerActive && (<Dialog open={scannerActive} onOpenChange={() => stopBarcodeScanner()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Scan Barcode</DialogTitle>
          </DialogHeader>
          <div ref={scannerRef} className="w-full h-64 bg-black rounded-lg overflow-hidden" data-testid="scanner-container"/>
          <DialogFooter>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={newDeptName} onChange={(e) => setNewDeptName(e.target.value)} placeholder="Enter department name" data-testid="input-dept-name"/>
            </div>
          </div>
          <DialogFooter>
            <Button onClick={() => {
      ...
      <Dialog open={showAddArea} onOpenChange={setShowAddArea}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">Name</label>
              <Input value={newAreaName} onChange={(e) => setNewAreaName(e.target.value)} placeholder="Enter area name" data-testid="input-area-name"/>
            </div>
          </div>
          <DialogFooter>
            <Button onClick={() => {
        ...
        }} disabled={!newAreaName.trim() || !selectedDepartment || createAreaMutation.isPending} data-testid="button-save-area">
