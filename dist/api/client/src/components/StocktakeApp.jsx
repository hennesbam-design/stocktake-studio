...
import { SearchIcon, ScanLineIcon, PlusIcon, ArrowLeftIcon, PackageIcon, EditIcon, BarChart3Icon } from "lucide-react";
...
export function StocktakeApp() {
    ...
    const productGroupsArray = Array.isArray(productGroupsData) ? productGroupsData : [];
    const filteredAreas = areasArray.filter((area) => selectedDepartment ? area.departmentId === selectedDepartment.id : true);
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                try {
                    ...
                    const missingHeaders = expectedHeaders.filter(header => !(header in firstRow) && !(`${header.trim()}` in firstRow));
    ...
    // Render functions
    const renderDepartmentView = () => (<div className="space-y-4">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">Departments</h1>
        <div className="flex gap-2">
          <input ref={csvInputRef} type="file" accept=".csv" onChange={handleCsvImport} className="hidden" data-testid="input-csv-import"/>
          <Button onClick={() => csvInputRef.current?.click()} variant="outline" disabled={csvImporting} data-testid="button-import-csv">
            {csvImporting ? (<div className="w-4 h-4 mr-2 animate-spin rounded-full border-2 border-transparent border-t-current"/>) : (<PlusIcon className="w-4 h-4 mr-2"/>)}
            Import CSV
          </Button>
          <Button onClick={() => setShowAddDept(true)} data-testid="button-add-department">
      ...
      {departmentsLoading ? (<div>Loading departments...</div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {departmentsArray.map((dept) => (<Card key={dept.id} className="hover-elevate cursor-pointer" onClick={() => {
              ...
              <CardContent>
                ...
                <Badge variant="secondary" className="mt-2">
                  {areasArray.filter((area) => area.departmentId === dept.id).length} areas
                </Badge>
              </CardContent>
            </Card>))}
        </div>)}
    </div>);
    const renderAreaView = () => (<div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setShowAddArea(true)} data-testid="button-add-area">
          <PlusIcon className="h-4 w-4 mr-2"/>
          Add Area
        </Button>
      </div>
      
      {areasLoading ? (<div>Loading areas...</div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAreas.map((area) => (<Card key={area.id} className="hover-elevate cursor-pointer" onClick={() => {
    ...
    const renderItemView = () => (<div className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={() => {
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={startBarcodeScanner} data-testid="button-scan-barcode">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search items by name or barcode..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" data-testid="input-search-items"/>
        </div>
      </div>
      
      {itemsLoading ? (<div>Loading items...</div>) : (<ScrollArea className="h-[600px]">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map((item) => (<Card key={item.id} className="hover-elevate cursor-pointer" onClick={() => {
                ...
                <CardContent className="space-y-2">
                  {item.barcode && (<Badge variant="outline" className="text-xs">
                      {item.barcode}
                    </Badge>)}
                  {item.packVolume && (<div className="flex items-center gap-1">
    ...
    const renderCountView = () => (<div className="space-y-4">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon" onClick={() => {
            setSelectedItem(null);
            setCurrentView('items');
      ...
      {selectedItem && (<Card>
          <CardHeader>
            ...
            {selectedItem.barcode && (<Badge variant="outline">{selectedItem.barcode}</Badge>)}
            {selectedItem.packVolume && (<div className="flex items-center gap-1">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="fulls" type="number" value={fulls} onChange={(e) => setFulls(e.target.value)} placeholder={`Default: ${selectedItem.defaultFulls || 0}`} data-testid="input-fulls"/>
              </div>
              <div>
                <Label htmlFor="singles">Singles</Label>
                <Input id="singles" type="number" value={singles} onChange={(e) => setSingles(e.target.value)} placeholder={`Default: ${selectedItem.defaultSingles || 0}`} data-testid="input-singles"/>
              </div>
            </div>
            
            <Button onClick={handleAddCount} className="w-full" data-testid="button-add-count">
              Add Count
            </Button>
          </CardContent>
        </Card>)}
    ...
    // Navigation
    const renderNavigation = () => (<div className="flex gap-2 mb-4">
      <Button variant={currentView === 'departments' ? 'default' : 'outline'} onClick={() => setCurrentView('departments')} data-testid="nav-departments">
        Departments
      </Button>
      <Button variant={currentView === 'reports' ? 'default' : 'outline'} onClick={() => setCurrentView('reports')} data-testid="nav-reports">
        <BarChart3Icon className="h-4 w-4 mr-2"/>
        Reports
      </Button>
    </div>);
    const renderReportsView = () => (<div className="space-y-4">
    ...
    return (<div className="min-h-screen bg-background">
      <div className="container mx-auto p-4">
        ...
        {scannerActive && (<Dialog open={scannerActive} onOpenChange={stopBarcodeScanner}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Scan Barcode</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                <div ref={scannerRef} className="w-full h-64 bg-black rounded relative overflow-hidden" style={{ position: 'relative' }}/>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="dept-name" value={newDeptName} onChange={(e) => setNewDeptName(e.target.value)} placeholder="Enter department name" data-testid="input-dept-name"/>
              </div>
            </div>
            <DialogFooter>
              <Button onClick={() => {
        ...
        }} disabled={!newDeptName.trim() || createDepartmentMutation.isPending} data-testid="button-save-dept">
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="area-name" value={newAreaName} onChange={(e) => setNewAreaName(e.target.value)} placeholder="Enter area name" data-testid="input-area-name"/>
              </div>
            </div>
            <DialogFooter>
              <Button onClick={() => {
        ...
        }} disabled={!newAreaName.trim() || createAreaMutation.isPending} data-testid="button-save-area">
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="item-name" value={newItemData.name} onChange={(e) => setNewItemData(prev => ({ ...prev, name: e.target.value }))} placeholder="Enter item name" data-testid="input-item-name"/>
              </div>
              <div>
                <Label htmlFor="item-barcode">Barcode</Label>
                <Input id="item-barcode" value={newItemData.barcode} onChange={(e) => setNewItemData(prev => ({ ...prev, barcode: e.target.value }))} placeholder="Enter barcode" data-testid="input-item-barcode"/>
              </div>
              <div>
                <Label htmlFor="item-pack-volume">Pack Volume</Label>
                <Input id="item-pack-volume" value={newItemData.packVolume} onChange={(e) => setNewItemData(prev => ({ ...prev, packVolume: e.target.value }))} placeholder="e.g., 500ml, 1kg, 12 pack" data-testid="input-item-pack-volume"/>
              </div>
            </div>
            <DialogFooter>
            ...
            if (newItemData.name.trim() && selectedArea) {
                try {
                    ...
                    let defaultGroup = productGroupsArray.find((group) => group.areaId === selectedArea.id && group.name === 'Default');
                    if (!defaultGroup) {
        ...
        }} disabled={!newItemData.name.trim() || createItemMutation.isPending} data-testid="button-save-item">
                Create Item
              </Button>
            </DialogFooter>
          </DialogContent>
